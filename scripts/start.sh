...
if "$PYTHON_CMD" tools/bootstrap_env.py --is-base-ready >/dev/null 2>&1; then
  echo "[env] Reusing ready project .venv"
else
  if ! "$PYTHON_CMD" tools/bootstrap_env.py --ensure-base; then
    echo "Failed to prepare the local .venv environment."
    exit 1
  fi
fi

VENV_PYTHON="$("$PYTHON_CMD" tools/bootstrap_env.py --print-python)"
